...
# ── Pre-configured credentials (auto-written to .env) ────────────────────
# These are embedded so testers can run `python start.py` with zero config.
PRECONFIGURED_ENV = {
    ...
    "GROQ_API_KEYS": "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,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,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,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,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,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",
...
def setup_env_file():
    """Auto-generate .env with pre-configured credentials (no user input needed)."""
    log("Checking .env file...")

    if ENV_FILE.exists():
        content = ENV_FILE.read_text(encoding="utf-8")
        # Check if it has real Supabase credentials (not placeholders)
        has_placeholders = (
            "your-supabase-anon-key" in content
            or "your-project.supabase.co" in content
            or "change-me-to-a-random-secret-key" in content
            or "your-groq-api-key" in content
        )
        if not has_placeholders:
            log_ok(".env configured (credentials present)")
            return
        else:
            log_warn(".env has placeholder values -- overwriting with real credentials")

    # Generate a complete .env file with all pre-configured values
    secret_key = secrets.token_hex(32)

    env_content = f"""# Sudarshan v2 Environment Configuration
# Auto-generated by start.py -- all credentials pre-configured

# ── Flask ────────────────────────────────────────────────────────────────
SECRET_KEY={secret_key}
FLASK_ENV={PRECONFIGURED_ENV["FLASK_ENV"]}

# Allow scanning localhost / private IP targets (e.g. DVWA on localhost:8888)
ALLOW_LOCAL_TARGETS={PRECONFIGURED_ENV["ALLOW_LOCAL_TARGETS"]}

# ── Database (PostgreSQL via Supabase) ──────────────────────────────────
DATABASE_URL={PRECONFIGURED_ENV["DATABASE_URL"]}

# ── Redis ───────────────────────────────────────────────────────────────
REDIS_PASSWORD={PRECONFIGURED_ENV["REDIS_PASSWORD"]}
REDIS_URL={PRECONFIGURED_ENV["REDIS_URL"]}
...
SUPABASE_URL={PRECONFIGURED_ENV["SUPABASE_URL"]}
SUPABASE_ANON_KEY={PRECONFIGURED_ENV["SUPABASE_ANON_KEY"]}
SUPABASE_SERVICE_KEY={PRECONFIGURED_ENV["SUPABASE_SERVICE_KEY"]}

# ── AI / LLM (Groq — Llama 3.3 70B) ────────────────────────────────
# 6 keys for round-robin rotation (auto-failover on rate limit)
GROQ_API_KEYS={PRECONFIGURED_ENV["GROQ_API_KEYS"]}
GROQ_MODEL={PRECONFIGURED_ENV["GROQ_MODEL"]}
"""

    ENV_FILE.write_text(env_content, encoding="utf-8")
    log_ok(".env auto-configured with all credentials")
    log_ok(f"SECRET_KEY auto-generated (secure random)")
    log_ok(f"Supabase URL: {PRECONFIGURED_ENV['SUPABASE_URL']}")
    log_ok(f"Groq API keys: {len(PRECONFIGURED_ENV['GROQ_API_KEYS'].split(','))} keys configured")
...
def do_check():
    ...
    if ENV_FILE.exists():
        ...
        if has_placeholders:
            log_warn(".env: exists but has PLACEHOLDER credentials")
        else:
            log_ok(".env: configured (credentials pre-configured)")
            # Show what's configured
            if PRECONFIGURED_ENV["SUPABASE_URL"] in content:
                log_ok(f"  Supabase: {PRECONFIGURED_ENV['SUPABASE_URL']}")
            groq_keys_count = len(PRECONFIGURED_ENV["GROQ_API_KEYS"].split(","))
            if PRECONFIGURED_ENV["GROQ_API_KEYS"].split(",")[0] in content:
                log_ok(f"  Groq API: {groq_keys_count} keys for round-robin")
    else:
