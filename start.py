...
# ── Supabase credentials (provided interactively or via environment) ──────
# SECURITY: Credentials are NEVER hardcoded. They are read from environment
# variables during CI, or prompted interactively during first-time setup.
# If you previously had hardcoded credentials here, rotate them immediately
# in the Supabase dashboard — they are compromised.
PRECONFIGURED_SUPABASE = {
    "DATABASE_URL": os.environ.get("SUDARSHAN_DATABASE_URL", ""),
    "SUPABASE_URL": os.environ.get("SUDARSHAN_SUPABASE_URL", ""),
    "SUPABASE_ANON_KEY": os.environ.get("SUDARSHAN_SUPABASE_ANON_KEY", ""),
    "SUPABASE_SERVICE_KEY": os.environ.get("SUDARSHAN_SUPABASE_SERVICE_KEY", ""),
}
...
def setup_env_file():
    """Auto-generate .env with Supabase credentials.
    
    Supabase details are read from environment variables (SUDARSHAN_*)
    or prompted interactively during first-time setup.
    """
    ...
    # Prompt for Supabase credentials if not provided via environment
    supabase_creds = dict(PRECONFIGURED_SUPABASE)
    if not supabase_creds["SUPABASE_URL"]:
        print()
        print(f"  {CYAN}{BOLD}--- SUPABASE CREDENTIALS ---{RESET}")
        print(f"  {DIM}  Enter your Supabase project credentials.{RESET}")
        print(f"  {DIM}  Get them from: https://supabase.com/dashboard/project/_/settings/api{RESET}")
        print(f"  {DIM}  Press Enter to skip (will use local SQLite database).{RESET}")
        print()
        supabase_creds["SUPABASE_URL"] = _ask("  SUPABASE_URL: ").strip()
        supabase_creds["SUPABASE_ANON_KEY"] = _ask("  SUPABASE_ANON_KEY: ").strip()
        supabase_creds["SUPABASE_SERVICE_KEY"] = _ask("  SUPABASE_SERVICE_KEY: ").strip()
        supabase_creds["DATABASE_URL"] = _ask("  DATABASE_URL (PostgreSQL connection string): ").strip()
        print()

    # Generate a complete .env
...
DATABASE_URL={supabase_creds["DATABASE_URL"]}
...
# ── Supabase Auth ──────────────────────────────────────────────────────
SUPABASE_URL={supabase_creds["SUPABASE_URL"]}
SUPABASE_ANON_KEY={supabase_creds["SUPABASE_ANON_KEY"]}
SUPABASE_SERVICE_KEY={supabase_creds["SUPABASE_SERVICE_KEY"]}
...
"""
    ...
    if supabase_creds["SUPABASE_URL"]:
        log_ok(f"Supabase URL: {supabase_creds['SUPABASE_URL']}")
    else:
        log_warn("No Supabase credentials — will use local SQLite database")
