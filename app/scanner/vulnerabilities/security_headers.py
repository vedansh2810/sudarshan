...
class SecurityHeadersScanner(BaseScanner):
    ...
    CSP_UNSAFE = [
        (
            "unsafe-inline",
            "script-src",
            "Allows inline scripts, defeating most XSS protections",
        ),
    ...
    def _check_cors(self, target_url):
        ...
        response = self._request(
            "GET", target_url, headers={"Origin": "https://evil-attacker.com"}
        )
        if not response:
            return findings
            ...
            findings.append(
                {
                    ...
                    "response_data": f"ACAO: {acao}"
                    + (f", ACAC: {acac}" if acac else ""),
    ...
    def _check_server_disclosure(self, response, target_url):
        ...
        if server and any(
            v in server.lower()
            for v in ["apache/", "nginx/", "iis/", "lighttpd/", "litespeed"]
        ):
            findings.append(
                {
                    "vuln_type": "security_headers",
