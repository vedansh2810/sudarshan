...
class CommandInjectionScanner(BaseScanner):
    ...
    def __init__(self, session=None, timeout=8, delay=0.5):
        ...
        try:
            ...
            logger.info(
                f"Command Injection: {count} payloads available ({len(extra)} new from PayloadManager)"
            )
        ...
        try:
            ...
            if smart:
                existing_set = set(
                    self.LINUX_OUTPUT_PAYLOADS + self.WINDOWS_OUTPUT_PAYLOADS
                )
                new_smart = [p for p in smart if p not in existing_set]
                self.LINUX_OUTPUT_PAYLOADS = (
                    list(self.LINUX_OUTPUT_PAYLOADS) + new_smart
                )
        except Exception:
            pass
    ...
    def _detect_target_os(self, url):
        ...
        try:
            ...
            if resp:
                ...
                if any(
                    w in server
                    for w in ("apache", "nginx", "unix", "ubuntu", "debian", "centos")
                ):
                    return "linux"
            ...
            if response:
                ...
                if found:
                    ...
                    if baseline_has_indicators:
                        ...
                        _, baseline_evidence = self._check_cmd_output(
                            baseline_resp.text
                        )
    ...
    def _test_form(self, form):
        ...
        testable = [
            ...
            for inp in inputs
            if inp["type"]
            not in ("submit", "button", "hidden", "csrf", "image", "reset")
        ]

        for target_input in testable:
            all_payloads = (
                self.LINUX_OUTPUT_PAYLOADS[:10] + self.WINDOWS_OUTPUT_PAYLOADS[:5]
            )
        ...
        for point in injectable_points:
            ...
            # ── URL params ──
                ...
                result = self._test_url_param(
                    url, param, params, parsed, target_os=target_os
                )
                if result:
                    seen.add(key)
                    self.findings.append(self._make_finding(result))
