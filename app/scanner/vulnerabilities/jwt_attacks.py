...
JWT_REGEX = re.compile(r"eyJ[A-Za-z0-9_-]{2,}\.eyJ[A-Za-z0-9_-]{2,}\.[A-Za-z0-9_-]*")
...
class JWTAttackScanner(BaseScanner):
    ...
    def _discover_jwts(self, target_url):
        ...
        for header_name in JWT_HEADERS:
            ...
            if match:
                jwts.append({"token": match.group(), "source": f"header:{header_name}"})
        ...
        if resp.text:
            for match in JWT_REGEX.finditer(resp.text):
                ...
                if token not in [j["token"] for j in jwts]:
                    jwts.append({"token": token, "source": "response_body"})
        ...
        for path in [
            ...
            if resp2 and resp2.text:
                for match in JWT_REGEX.finditer(resp2.text):
                    token = match.group()
                    if token not in [j["token"] for j in jwts]:
                        jwts.append({"token": token, "source": f"endpoint:{path}"})
        ...
        if source.startswith("cookie:"):
            ...
            if header_name == "Authorization":
                return self._request(
                    "GET", target_url, headers={"Authorization": f"Bearer {token}"}
                )
