...
class XSSScanner(BaseScanner):
    ...
    def __init__(self, session=None, timeout=8, delay=0.05):
        ...
        try:
            ...
            logger.info(
                f"XSS: {count} payloads available ({len(extra)} new from PayloadManager)"
            )
    ...
    def _check_reflected(self, response_text, payload):
        ...
        if has_html_chars:
            ...
            if payload in response_text:
                ...
                if (
                    encoded_payload in response_text
                    and payload
                    not in response_text.replace("&lt;", "<").replace("&gt;", ">")
                ):
            ...
            if self.MARKER in response_text:
                ...
                if idx >= 0:
                    context = response_text[
                        max(0, idx - 50) : idx + len(self.MARKER) + 50
                    ]
    ...
    def _test_param(self, url, param_name):
        ...
        if context == "attribute":
            payloads = [
                p.replace("{m}", m)
                for p in self.ATTR_PAYLOADS + self.EVENT_PAYLOADS[:6]
            ]
        elif context == "script":
            payloads = [
                p.replace("{m}", m)
                for p in self.JS_CONTEXT_PAYLOADS + self.BASIC_PAYLOADS[:2]
            ]
        elif context == "url":
            payloads = [
                p.replace("{m}", m)
                for p in self.URL_CONTEXT_PAYLOADS + self.ATTR_PAYLOADS[:3]
            ]
        else:
                        ...
                        vulnerabilities.append(
                            {
                                ...
                                "source": source,
                                "sink": sink,
                                "evidence": DOMXSSAnalyzer._extract_snippet(
                                    javascript_code, source, sink
                                ),
