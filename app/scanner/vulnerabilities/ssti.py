...
class SSTIScanner(BaseScanner):
    ...
    def _test_expression_probes(self, url, param_name, params, method):
        ...
        baseline = self._request(
            method, url, **self._build_kwargs(method, baseline_params)
        )
    ...
    def _build_finding(
        self, url, param, payload, engine, technique, evidence_text, method
    ):
        evidence_snippet = (
            (evidence_text[:300] + "...") if len(evidence_text) > 300 else evidence_text
        )
