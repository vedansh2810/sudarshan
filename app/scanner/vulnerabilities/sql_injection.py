...
class SQLInjectionScanner(BaseScanner):
    ...
    def __init__(self, session=None, timeout=8, delay=0.05):
        ...
        try:
            ...
            logger.info(
                f"SQL Injection: {count} payloads available ({len(extra)} new from PayloadManager)"
            )
    ...
    def _test_boolean_blind(self, url, param_name, params, parsed):
        ...
        for true_payload, false_payload in self.BOOLEAN_TRUE:
            ...
            if len_diff > max(200, baseline_jitter * 3) and (
    ...
    def _validate_finding(self, url, param_name, params, parsed, payload, technique):
        ...
        if technique == "error-based":
            alt_payload = (
                payload.replace("'", '"') if "'" in payload else payload + " --"
            )
        ...
        injected = self._request(
            "GET", self._build_url(parsed, params, param_name, payload)
        )
        ...
        if injected:
            kw_count = sum(
                1
                for kw in ("SELECT", "FROM", "WHERE", "syntax", "query", "SQL")
