...
class Scan:
    ...
    def for_user_query(user_id):
        ...
        if org_ids:
            return ScanModel.query.filter(
                or_(ScanModel.user_id == user_id, ScanModel.org_id.in_(org_ids))
            )
    ...
    def get_stats(user_id):
        ...
        vulns = (
            ...
            .filter(ScanModel.id.in_(db.session.query(accessible_ids.c.id)))
            .first()
        )
    ...
    def update_status(scan_id, status):
        ...
        db.session.query(ScanModel).filter_by(id=scan_id).update({"status": status})
        db.session.commit()
    ...
    def get_logs(scan_id):
        logs = (
            ScanLogModel.query.filter_by(scan_id=scan_id)
            .order_by(ScanLogModel.logged_at.asc())
            .all()
        )
        return [
            {c.name: getattr(l, c.name) for c in ScanLogModel.__table__.columns}
            for l in logs
        ]
    ...
    def add_logs_batch(scan_id, messages):
        ...
        for msg in messages:
            if isinstance(msg, tuple):
                logs.append(
                    ScanLogModel(scan_id=scan_id, message=msg[0], log_type=msg[1])
                )
            else:
                logs.append(
                    ScanLogModel(scan_id=scan_id, message=str(msg), log_type="info")
                )
    ...
    def recover_orphaned(max_age_minutes=10):
        ...
        orphaned = ScanModel.query.filter(
            ScanModel.status.in_(["running", "pending"]), ScanModel.started_at < cutoff
        ).all()
        for scan in orphaned:
            ...
            db.session.add(log)
            logger.warning(f"Recovered orphaned scan {scan.id} (was '{original_status}')")
        if orphaned:
            db.session.commit()
