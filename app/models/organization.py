...
class OrgMembershipModel(db.Model):
    ...
    user = db.relationship(
        "UserModel", backref=db.backref("memberships", lazy="dynamic")
    )
...
class OrgSettingsModel(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    org_id = db.Column(
        db.Integer, db.ForeignKey("organizations.id"), unique=True, nullable=False
    )
    max_scans_per_month = db.Column(
        db.Integer, nullable=True
    )  # None = use plan default
    allowed_domains = db.Column(
        db.Text, nullable=True
    )  # JSON list of allowed target domains
...
class Organization:
    ...
    def create(name, owner_user_id):
        ...
        try:
            ...
            db.session.commit()
            logger.info(
                f"Created org '{name}' (slug={slug}) with owner user_id={owner_user_id}"
            )
    ...
    def get_user_org_ids(user_id):
        ...
        rows = (
            db.session.query(OrgMembershipModel.org_id).filter_by(user_id=user_id).all()
        )
    ...
    def remove_member(org_id, user_id):
        ...
        m = OrgMembershipModel.query.filter_by(user_id=user_id, org_id=org_id).first()
        if m:
            db.session.delete(m)
            db.session.commit()
            return True
    ...
    def user_has_access(org_id, user_id):
        ...
        return (
            OrgMembershipModel.query.filter_by(user_id=user_id, org_id=org_id).first()
            is not None
        )
    ...
    def delete_all_data(org_id):
        ...
        try:
            ...
            member_ids = [
                m.user_id
                for m in OrgMembershipModel.query.filter_by(org_id=org_id).all()
            ]
            if member_ids:
                from app.models.webhook import Webhook

                Webhook.query.filter(Webhook.user_id.in_(member_ids)).delete(
    ...
    def check_scan_quota(org_id):
        ...
        current_count = ScanModel.query.filter(
            ScanModel.org_id == org_id, ScanModel.started_at >= month_start
        ).count()

        if current_count >= max_scans:
            return (
                False,
                f"Monthly scan quota reached ({current_count}/{max_scans}). Upgrade your plan.",
            )
        return True, ""
