...
def dashboard():
    ...
    if total_vulns > 0:
        sev_pct = {
            k: round(stats[k] / total_vulns * 100)
...
def list_scans():
    ...
    scans_orm = (
        query.order_by(ScanModel.started_at.desc()).limit(per_page).offset(offset).all()
    )
...
def download_report(scan_id, fmt):
    ...
    if fmt == "pdf":
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=sudarshan-report-{scan_id}.pdf"
            },
        )
    else:
        ...
        return Response(
            ...
            mimetype="text/html",
            headers={
                "Content-Disposition": f"attachment; filename=sudarshan-report-{scan_id}.html"
            },
        )
