...
def _generate_pdf_report(scan, vulns, ai_summary=None):
    ...
    for label, count, color in [
        ...
        ("High", high, (234, 88, 12)),
        ("Medium", medium, (202, 138, 4)),
        ("Low/Info", low, (22, 163, 74)),
    ]:
    ...
    info_rows = [
        ...
        (
            "Completed",
            str(s.get("completed_at", ""))[:16] if s.get("completed_at") else "N/A",
        ),
    ]
    ...
    if vulns:
        ...
        for idx, v in enumerate(vulns, 1):
            ...
            pdf.cell(
                col_widths[2], 6, _safe(v["severity"]).upper(), border=1, align="C"
            )
            pdf.set_text_color(30, 30, 30)
            pdf.set_font("Helvetica", "", 7)
            pdf.cell(
                col_widths[3], 6, str(v.get("cvss_score", "")), border=1, align="C"
            )
            pdf.cell(
                col_widths[4], 6, _safe(v.get("owasp_category", ""))[:22], border=1
            )
    ...
    for idx, v in enumerate(vulns, 1):
        ...
        pdf.set_text_color(*color)
        pdf.multi_cell(
            full_w, 7, f"{idx}. {_safe(v['name'])}  [{_safe(v['severity']).upper()}]"
        )
    ...
    pdf.cell(
        0,
        5,
        f"Generated by Sudarshan Web Vulnerability Scanner  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}  |  For authorized use only",
        align="C",
    )
