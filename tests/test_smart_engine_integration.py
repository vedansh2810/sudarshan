...
class TestSmartEnginePortSwigger:
    ...
    def test_portswigger_context_returns_data(self):
        ...
        engine = SmartEngine()
        ctx = engine.get_portswigger_context(
            "sql_injection", max_labs=2, max_payloads=3
        )
...
class TestSmartEngineLLMGraceful:
    ...
    def test_reconnaissance_without_llm(self):
        ...
        result = engine.reconnaissance(
            "http://example.com",
            MagicMock(headers={}, text="<html></html>", status_code=200),
        )
...
class TestBaseScanner:
    ...
    def test_ai_verify_finding_returns_triple(self):
        ...
        result = scanner._ai_verify_finding(
            {"vuln_type": "sql_injection"}, None, None, "' OR 1=1"
        )
        assert isinstance(result, tuple)
        assert len(result) == 3
