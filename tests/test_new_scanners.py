...
class TestXXEScanner:
    ...
    def test_xxe_no_false_positive_on_safe_xml(self):
        ...
        mock_resp.status_code = 200
        mock_resp.text = (
            "<response><status>ok</status><data>Hello World</data></response>"
        )
        mock_resp.headers = {"Content-Type": "application/xml"}
    ...
    def test_xxe_endpoint_detection(self):
        ...
        assert scanner._is_xml_endpoint(
            {"name": "payload"}, "http://target.com/api/parse"
        )
        assert not scanner._is_xml_endpoint(
            {"name": "username"}, "http://target.com/login"
        )
...
class TestSSRFScanner:
    ...
    def test_ssrf_localhost_detection(self):
        ...
        injectable = [
            {
                "name": "target",
                "url": "http://target.com/proxy?target=http://example.com",
            }
        ]
    ...
    def test_ssrf_no_false_positive(self):
        ...
        mock_resp.status_code = 200
        mock_resp.text = (
            "<html><body><h1>Hello World</h1><p>Regular content here</p></body></html>"
        )
        mock_resp.headers = {}
...
class TestOpenRedirectScanner:
    ...
    def test_open_redirect_detection(self):
        ...
        injectable = [
            {"name": "redirect", "url": "http://target.com/login?redirect=/dashboard"}
        ]
