...
def add_centered_text(
    text, size=12, bold=False, italic=False, space_after=0, space_before=0
):
    p = doc.add_paragraph()
...
def add_chapter_title(text):
    ...
    pBdr = parse_xml(
        f'<w:pBdr {nsdecls("w")}><w:bottom w:val="single" w:sz="18" w:space="1" w:color="000000"/></w:pBdr>'
    )
    pPr.append(pBdr)
...
add_justified_para(
    "The primary objectives of this project are as follows:", indent_first=False
)
add_bullet(
    "To design and implement a full-stack web vulnerability scanner capable of detecting sixteen distinct vulnerability types conforming to OWASP guidelines."
)
add_bullet(
    "To integrate a large language model (Llama 3.3 70B) for context-aware reconnaissance, payload generation, and vulnerability analysis."
)
add_bullet(
    "To build a machine-learning false-positive classifier using Random Forest and Gradient Boosting ensemble techniques."
)
add_bullet(
    "To incorporate a curated PortSwigger knowledge base with 269 labs and 2,197 payloads for expert-level attack strategies."
)
add_bullet(
    "To develop a multi-tenant SaaS architecture supporting organizations, role-based access, and API key authentication."
)
add_bullet(
    "To generate AI-powered security reports with executive summaries, remediation plans, and attack narratives."
)
add_bullet(
    "To containerize the entire application stack using Docker for reproducible deployment."
)
...
add_justified_para(
    "Based on the above literature survey, the following gaps were identified:",
    indent_first=False,
)
add_bullet(
    "No open-source scanner combines LLM reasoning with structured knowledge bases and ML classification in a unified architecture."
)
add_bullet(
    "Existing tools treat AI as an afterthought rather than a core component of the scanning pipeline."
)
add_bullet(
    "False-positive filtering in most scanners is rule-based rather than learned from data."
)
add_bullet(
    "Report generation is typically template-based, producing generic outputs that lack contextual depth."
)
add_justified_para(
...
add_figure(
    os.path.join(DIAGRAM_DIR, "system_architecture.png"),
    "Sudarshan System Architecture",
    "3.1",
)
...
add_figure(
    os.path.join(DIAGRAM_DIR, "er_diagram.png"),
...
add_figure(
    os.path.join(DIAGRAM_DIR, "scan_pipeline.png"), "Scan Pipeline Data Flow", "3.3"
)
