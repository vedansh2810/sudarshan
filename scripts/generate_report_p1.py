...
rFonts = parse_xml(
    f'<w:rFonts {nsdecls("w")} w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:eastAsia="Times New Roman" w:cs="Times New Roman"/>'
)
rPr.append(rFonts)
...
def add_centered_text(
    text, size=12, bold=False, italic=False, color=None, space_after=0, space_before=0
):
    p = doc.add_paragraph()
...
# ═══════════════════════════════════════
add_centered_text(
    "FACULTY OF COMPUTER SCIENCE & ENGINEERING", 12, bold=True, space_after=2
)
...
add_centered_text(
    "Plot No. 2027-2031, Ramchandrapura P.O. Vidhani Vatika Road,", 10, space_after=1
)
...
add_centered_text(
    "Poornima University, Jaipur (Academic Session: 2025-26)", 12, space_after=18
)
...
add_centered_text(
    "Faculty of Computer Science & Engineering, Poornima University", 12, space_after=2
)
