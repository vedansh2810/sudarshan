...
OUT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "data", "report_diagrams"
)
...
def gen_architecture():
    ...
    draw_box(
        ax, 8.4, 4.1, 2.2, 0.55, "Groq LLM\n(Llama 3.3 70B)", WHITE, PURPLE, 7, False
    )
    draw_box(
        ax, 6.0, 3.45, 2.2, 0.5, "PortSwigger KB\n(269 Labs)", WHITE, PURPLE, 7, False
    )
    draw_box(
        ax, 8.4, 3.45, 2.2, 0.5, "ML FP Classifier\n(RF + GB)", WHITE, PURPLE, 7, False
    )
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(OUT_DIR, "system_architecture.png"), dpi=200, bbox_inches="tight"
    )
    plt.close(fig)
...
def gen_pipeline():
    ...
    ax.axis("off")
    ax.set_title(
        "Scan Pipeline — Data Flow", fontsize=14, fontweight="bold", pad=15, color=NAVY
    )
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(OUT_DIR, "scan_pipeline.png"), dpi=200, bbox_inches="tight"
    )
    plt.close(fig)
    print("✓ Scan Pipeline")
...
def gen_er():
    ...
    draw_table(
        ...
        8.2,
        "users",
        ["PK id", "supabase_uid", "username", "email", "is_admin"],
    )
    # Scans
    draw_table(
        ax,
    ...
    draw_table(
        ...
        [
            ...
            "FK scan_id → scans",
            "vuln_type / severity",
            "cvss_score / owasp_category",
            "affected_url / parameter",
    ...
    draw_table(
        ...
        5.8,
        "organizations",
        ["PK id", "name / slug", "plan (free/pro/enterprise)"],
    )
    # Org Memberships
    draw_table(
        ax,
        ...
        4.5,
        "org_memberships",
        ["FK user_id → users", "FK org_id → organizations", "role"],
    )
    # Crawled URLs
    draw_table(
        ax,
        ...
        5.5,
        "crawled_urls",
        ["PK id", "FK scan_id → scans", "url / status_code", "forms_found"],
    )
    # Scan Logs
    draw_table(
        ax, 4.5, 4.0, "scan_logs", ["PK id", "FK scan_id → scans", "log_type / message"]
    )
    # Webhooks
    draw_table(
        ax,
        ...
        5.5,
        "webhooks",
        ["PK id", "FK user_id → users", "url / is_active", "failure_count"],
    )
    # API Keys
    draw_table(
        ax,
...
def gen_ai_ml():
    ...
    ax.text(
        1.7, 5.0, "Groq LLM", ha="center", fontsize=10, fontweight="bold", color=BLUE
    )
    ax.text(1.7, 4.6, "Llama 3.3 70B Versatile", ha="center", fontsize=7, color=DARK)
    ax.text(
        1.7,
        4.25,
        "Rate Limited · Cached · Fallback",
        ha="center",
        fontsize=6.5,
        color="#6b7280",
    )
    ...
    ax.text(
        5.5,
        2.45,
        "16 Features · Trained from labeled scan_attempts",
        ha="center",
        fontsize=6.5,
        color="#6b7280",
    )
    ...
    ax.text(
        5.5,
        0.9,
        "ML Score (40% weight)  +  LLM Score (60% weight)  =  Final Verdict",
        ha="center",
        fontsize=8,
        color=DARK,
    )
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(OUT_DIR, "ai_ml_integration.png"), dpi=200, bbox_inches="tight"
    )
    plt.close(fig)
